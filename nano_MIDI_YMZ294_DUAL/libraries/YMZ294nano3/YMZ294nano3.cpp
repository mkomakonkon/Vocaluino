...
を修正し、MIDI対応、YMZ294x2個対応しています。
Ver3:高速化のため全てポート制御に変更
*/
...
// Constructor
/* ------------------------------------------------------------------------ */
YMZ294nano3::YMZ294nano3(byte wrcs0_pin, byte wrcs1_pin, byte a0_pin, byte reset_pin)
...
// Channel On/Off
void YMZ294nano3::SetEnable(Channel ch, bool enable)
{
	if(ch <=2){
		if(enable)
...
void YMZ294nano3::SetFreqBit(Channel ch, word freqbit)
{
	if(ch <=2){
...
void YMZ294nano3::SetFrequency(Channel ch, word freq)
{
	SetFreqBit(ch, (BASE_FREQ / freq));
}

// Set Volume (4bit)
void YMZ294nano3::SetVolume(Channel ch, byte volume)
{
	if(ch <=2){
		volumes[ch] = volume;
...
void YMZ294nano3::SetEnvEnable(Channel ch, bool enable)
{
	if(ch <=2){
...
void YMZ294nano3::SetEnvFreqBit(word freqbit)
{
	env_freqbit = freqbit;
...
void YMZ294nano3::SetEnvFrequency(word freq)
{
	SetEnvFreqBit(BASE_FREQ / freq);
}

// Set Envelope Shape (4bit)
void YMZ294nano3::SetEnvShape(bool cont, bool att, bool alt, bool hold)
{
	env_shape = (cont<< 3) |
				(att << 2) |
				(alt << 1) |
...
void YMZ294nano3::SetNoiseFreqBit(byte freqbit)
{
...
void YMZ294nano3::SetNoiseFrequency(word freq)
{
	SetNoiseFreqBit(BASE_FREQ / freq);
}

// ------ Mixer Functions ---------------------------------------- */
void YMZ294nano3::SetMixer(byte noise, byte square)
{
...
/* ------------------------------------------------------------------------ */
void YMZ294nano3::param_reset(void)
{
...
void YMZ294nano3::hardware_reset(void)
{
    PORTD |= _BV(2);	//digitalWrite(WRCS0_PIN, HIGH);
    PORTD |= _BV(3);	//digitalWrite(WRCS1_PIN, HIGH);
    PORTD &= ~_BV(4);	//digitalWrite(A0_PIN, LOW);
    PORTD &= ~_BV(5);	//digitalWrite(RESET_PIN, LOW);
	delay(10);
    PORTD |= _BV(5);	//digitalWrite(RESET_PIN, HIGH);
}

void YMZ294nano3::set_register(byte addr, byte value)
{
  // addr
    PORTD &= ~_BV(2);	//digitalWrite(WRCS0_PIN, LOW);
    PORTD &= ~_BV(3);	//digitalWrite(WRCS1_PIN, LOW);
    PORTD &= ~_BV(4);	//digitalWrite(A0_PIN, LOW);
  PORTB = addr;
  PORTC = addr >> 2;
    PORTD |= _BV(2);	//digitalWrite(WRCS0_PIN, HIGH);
  ...
  // value
    PORTD &= ~_BV(2);	//digitalWrite(WRCS0_PIN, LOW);
    PORTD &= ~_BV(3);	//digitalWrite(WRCS1_PIN, LOW);
    PORTD |= _BV(4);	//digitalWrite(A0_PIN, HIGH);
  PORTB = value;
  PORTC = value >> 2;
    PORTD |= _BV(2);	//digitalWrite(WRCS0_PIN, HIGH);
    PORTD |= _BV(3);	//digitalWrite(WRCS1_PIN, HIGH);
...
void YMZ294nano3::set_register0(byte addr, byte value)
{
  // addr
    PORTD &= ~_BV(2);	//digitalWrite(WRCS0_PIN, LOW);
    PORTD &= ~_BV(4);	//digitalWrite(A0_PIN, LOW);
...
void YMZ294nano3::set_register1(byte addr, byte value)
{
  // addr
    PORTD &= ~_BV(3);	//digitalWrite(WRCS1_PIN, LOW);
    PORTD &= ~_BV(4);	//digitalWrite(A0_PIN, LOW);
