 ...
 *        Expression対応
 * V03.02 PitchBend対応
 * V03.03 PitchBendバグ修正
 * V03.04 Velocity対応
 * V03.05 自ChのみRead(ノイズ対策)
 * V03.06 Volume設定時にExpression=127とする
 * 　　　（VolumeとExpressionを区別していないデータ用）
 *        ResetAllControllersでnanoの変数初期化
 */
...
const int MIDI_IN_CHANNEL = 16; //★ここで演奏したいMIDIチャンネルを設定★
int BEFORE_NOTE_A = 0;
int BEFORE_NOTE_B = 0;
int BEFORE_NOTE_C = 0;
int BEFORE_NOTE_D = 0;
...
int pitch_val1  = 0; //MIDI PitchBend Value (Default = 0)
int pitch_val2  = 64; //MIDI PitchBend Value (Default = 64)
int ymz_pitch = 0; //PitchBend for YMZ (Default = min 0)
int vel  = 127; //MIDI Velocity Value (Max = 127)
...
void set_ch(Channel ch, int note) {
 ...
 ymz.SetVolume(ch, ymz_vol*vel/127);
...
void loop() {
 ...
 vel = MIDI.getData2(); // velocity value
 if(BEFORE_NOTE_A == 0) {
 BEFORE_NOTE_A = note;
 ch = CH_A;
 ...
 case midi::PitchBend:
 pitch_val1 = MIDI.getData1(); // PitchBend value
 pitch_val2 = MIDI.getData2(); // PitchBend value
 ymz_pitch = ((pitch_val2 << 7) + pitch_val1 - 8192)/682;
 if(BEFORE_NOTE_A != 0) {
 ...
 if(mdata == 11) {        // mdata=Expression
  ...
  ex_val = 127; //VolumeとExpressionを区別していないデータ用
...
//-- reset for Arduino value
 ymz_vol = 15; //MIDI Volume (Default = Max 15)
 ex_val  = 127; //MIDI Expression Value (Default = Max 127)
 pitch_val1  = 0; //MIDI PitchBend Value (Default = 0)
 pitch_val2  = 64; //MIDI PitchBend Value (Default = 64)
 ymz_pitch = 0; //PitchBend for YMZ (Default = min 0)
 vel  = 127; //MIDI Velocity Value (Max = 127)
 }
 break;
 default:
 break;
